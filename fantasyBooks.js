...
const FantasyBooks = () => {
  ...
  useEffect(() => {
    const fetchFantasyBooks = async () => {
      const response = await fetch('https://www.googleapis.com/books/v1/volumes?q=subject:Fantasy%20stories+Adult&langRestrict&maxResults=40');
      const data = await response.json();
      const newFantasyBooks = data.items.filter((book) => {
        return !books.some((existingBook) => existingBook.id === book.id);
      });
      setNewBooks([...books, ...newFantasyBooks]);
    }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.upperPart}>
        <Text style={styles.text}> Fantasy Books </Text>
