...
const FantasyBooks = () => {
  ...
  useEffect(() => {
    const fetchThrillerBooks = async () => {
      const response = await fetch('https://www.googleapis.com/books/v1/volumes?q=subject:Horror&langRestrict=en&orderBy=newest&printType=books&maxResults=40');
      const data = await response.json();
      const newThrillerBooks = data.items.filter((book) => {
        return !books.some((existingBooks) => existingBooks.id === book.id);
      })
      setNewBooks([...books, ...newThrillerBooks]);
    }
    fetchThrillerBooks();
  }, []);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.upperPart}>
        <Text style={styles.text}> Thriller Books </Text>
