...
const FantasyBooks = () => {
  ...
  useEffect(() => {
    const fetchScFBooks = async () => {
      const response = await fetch('https://www.googleapis.com/books/v1/volumes?q=subject:Fiction&langRestrict=en&orderBy=newest&printType=books&maxResults=40');
      const data = await response.json();
      const newScfBooks = data.items.filter((book) => {
        return !books.some((existingBooks) => existingBooks.id === book.id);
      })
      setNewBooks([...books, ...newScfBooks]);
    }
    fetchScFBooks();
  }, []);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.upperPart}>
        <Text style={styles.text}> Science Fiction Books </Text>
