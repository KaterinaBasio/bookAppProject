...
const PopularBooks = () => {
  ...
  useEffect(() => {
    const fetchPopularBooks = async () => {
      const response = await fetch('https://www.googleapis.com/books/v1/volumes?q=novel+year:2022&printType=books&maxResults=30');
      const data = await response.json();
      const newPopularBooks = data.items.filter((book) => {
        return !books.some((existingBook) => existingBook.id === book.id);
      });
      setNewBooks([...books, ...newPopularBooks]);
    }
  ...
  return (
    ...
    <View style={styles.container}>
      ...
      <Text style={styles.text}> Popular Books 2022 </Text>
