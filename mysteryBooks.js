...
const FantasyBooks = () => {
  ...
  useEffect(() => {
    const fetchMysteryBooks = async () => {
      const response = await fetch('https://www.googleapis.com/books/v1/volumes?q=subject:mystery&langRestrict=en&orderBy=newest&printType=books&maxResults=40')
      const data = await response.json();
      const newMysteryBooks = data.items.filter((book) => {
        return !books.some((existingBook) => existingBook.id === book.id)
      })
      setNewBooks([...books, ...newMysteryBooks]);
    }
    fetchMysteryBooks();
},[]);
  ...
  return (
    <View style={styles.container}>
      <View style={styles.upperPart}>
        <Text style={styles.text}> Mystery Books </Text>
